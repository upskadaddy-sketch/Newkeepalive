...
def classify_error(e):
    msg=str(e).lower()
    if "version_code" in msg or "app_version" in msg:
        return "version_code" # SPECIAL FIX
    if isinstance(e,(LoginRequired,ChallengeRequired)) or "login_required" in msg or "challenge" in msg or "logged out" in msg or "session expired" in msg or "401" in msg or "403" in msg:
        return "auth"
    if "429" in msg or "rate limit" in msg or isinstance(e,PleaseWaitFewMinutes):
        return "rate_limit"
    return "transient"
...
# FIXED LOGIN - version_code error fixed
def login_session(session_id, name_hint=""):
    ...
    for device_try in range(2): # try 2 devices if version_code fails
        try:
            cl=Client(request_timeout=20)
            # FIX 1: NEW APP VERSION - old 269 version gives version_code error in 2026
            if device_try==0:
                # Latest working device
                cl.set_device({
                    "app_version": "304.0.0.38.104",
                    "android_version": 33,
            ...
            if proxy:
                cl.set_proxy(proxy)
                log(f"🌐 Proxy set {acc_name}",session=acc_name)
            # Login
            cl.login_by_sessionid(session_id)
            # Safe check
            cl.get_timeline_feed()
            log(f"✅ Logged in {getattr(cl,'username','?')} - ID SAFE [{cl.device.get('app_version')}]",session=acc_name)
            return cl
        except Exception as e:
            err=classify_error(e)
            log(f"❌ Login attempt {device_try+1} failed ({acc_name}) [{err}]: {e}",session=acc_name)
            if err=="version_code":
                log(f"🔄 Trying fallback device for {acc_name}...",session=acc_name)
                time.sleep(2)
                continue
            return None
    return None
...
def safe_send(cl,gid,msg,acc):
    for attempt in range(3):
        try:
            ...
            err=classify_error(e)
            log(f"⚠ Send [{err}] att {attempt+1} {acc}->{gid}: {e}",session=acc)
            if err=="auth": return False
            if err=="rate_limit": time.sleep(30); return False
...
def safe_title(cl,gid,title,acc):
    try:
        ...
        log(f"📝 {getattr(cl,'username','?')} title {gid}->{title}",session=acc)
        return True
    except:
        try:
            cl.private.headers.update({"User-Agent":"Mozilla/5.0","X-CSRFToken":CSRF_TOKEN,"X-Requested-With":"XMLHttpRequest","Referer":f"https://www.instagram.com/direct/t/{gid}/"})
            cl.private.cookies.update({"csrftoken":CSRF_TOKEN})
            ...
            if "errors" not in resp.json():
                log(f"📝 {getattr(cl,'username','?')} title graphql {gid}->{title}",session=acc)
                return True
        except Exception as e:
            log(f"❌ Title fail {gid}: {e}",session=acc)
    return False
        ...
        try:
            if acc.get("cooldown_until",0)>time.time():
                ...
                for _ in range(BURST_COUNT):
                    for gid in groups:
                        ok=safe_send(cl,gid,MESSAGE_TEXT,acc_name)
                        if not ok:
            ...
            log(f"❌ msg loop {acc_name}: {e}",session=acc_name)
    ...
    while True:
        ...
        try:
            if not acc["active"] or not acc["client"]:
                log(f"⏭ {acc['name']} nc inactive",session=acc["name"])
            else:
                cl=acc["client"]
                for gid in groups:
                    t=titles_map.get(str(gid)) or titles_map.get(int(gid)) or [titles[idx%len(titles)]]
                    if isinstance(t,list): t=t[0]
                    safe_title(cl,gid,t,acc["name"])
                    time.sleep(1)
        except Exception as e:
            log(f"❌ nc loop {acc['name']}: {e}",session=acc["name"])
    ...
    while True:
        if SELF_URL:
            try:
                ...
                log(f"🔁 Self ping OK",session="system")
            except Exception as e:
...
def summarize(lines):
    ...
    return {
        ...
        "last_send_error": next((l for l in rev if "Send failed" in l or "⚠" in l), None),
        "last_title_ok": next((l for l in rev if "📝" in l), None),
        "last_title_error": next((l for l in rev if "Title change" in l or "GraphQL" in l), None),
        "total": len(lines)
    }
...
DASHBOARD_HTML="""
<!doctype html>
<html><head><title>FINAL - NO LOGOUT</title><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="15">
<style>
...
.green{color:#0f0} .red{color:#f55} .blue{color:#0af}
...
<h2>🚀 FINAL PANEL - Logout Fixed (version_code fixed)</h2>
<p>Time: {{now}} | Self: {{self_url}} | Ping: {{ping_interval}}s | <a href="/status">/status</a> | <a href="/health">/health</a> | <a href="/ping">/ping</a></p>
<div class="card"><b>Config:</b> Groups={{group_count}} | Msg="{{msg_text}}" | SpamGap={{spam_gap}}s NC_Gap={{nc_gap}}s | Proxy={{'SET ✅' if proxy_set else 'NOT SET (bina proxy ke 403 ayega)'}}<br><small>Fix: app_version 304.0.0.38.104 (version_code fix), device fixed, timeline_feed safe, proxy support</small></div>
{% for acc in accounts %}
<div class="card">
<h3>{{acc.name}} - <span class="{{'green' if acc.active else 'red'}}">{{'ACTIVE SAFE ✅' if acc.active else 'INACTIVE ❌'}}</span> {% if acc.active %}<span class="badge ok">NO LOGOUT</span>{% else %}<span class="badge fail">LOGIN FAIL - check logs</span>{% endif %}</h3>
<small>Proxy: {{acc.proxy_status}} | Cooldown: {{acc.cooldown_fmt}}</small>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:6px">
<div><b>Last Login (time/date)</b><div class="log">{{acc.summary.last_login or 'none - session id check karo'}}</div><b>Last Send OK</b><div class="log green">{{acc.summary.last_send_ok or 'none'}}</div><b>Send Err</b><div class="log red">{{acc.summary.last_send_error or 'none'}}</div></div>
<div><b>Last NC OK</b><div class="log">{{acc.summary.last_title_ok or 'none'}}</div><b>Logs</b><div class="log">{{acc.logs_tail}}</div></div>
</div></div>
{% endfor %}
<div class="card"><h3>System - Ping / Time Logs</h3><div class="log">{{system_logs}}</div></div>
</body></html>
"""
...
def dashboard():
    with logs_lock:
        ...
        for i in range(1,7):
            ...
            acc_data.append({
                "name":name,"active":acc_obj.get("active",False),
                ...
                "proxy_status":"SET" if get_proxy(name) else "NOT SET",
...
def start_bot():
    ...
    log(f"STARTUP: groups={len(GROUP_IDS.split(','))} msg={MESSAGE_TEXT[:20]}",session="system")
    ...
    for i,s in enumerate(sessions,1):
        name=f"acc{i}"
        if not s:
            accounts.append({"name":name,"client":None,"active":False,"cooldown_until":0,"raw_session":None})
            continue
    ...
    if not any(a["client"] for a in accounts):
        log("❌ No login - SESSION_ID check karo, naya session_id lo",session="system"); return
    ...
    log("▶ All loops started - version_code FIXED",session="system")
